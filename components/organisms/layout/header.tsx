            ...
            {menuOpen && (
              <div className="absolute right-6 top-16 z-40 w-56 rounded-xl border border-border bg-background shadow-lg py-2">
                <div className="px-4 py-2 border-b border-border">
                  <p className="text-xs uppercase tracking-wider text-foreground/60">Signed in as</p>
                  <p className="text-sm font-semibold text-foreground">{session?.role ?? "Admin"}</p>
                </div>
                <button
                  type="button"
  ...
  return (
    <header className="border-b border-border bg-background">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
        <Link href="/" className="text-xl font-bold text-foreground">P!NGFEST</Link>
        <nav className="hidden md:flex items-center gap-8">
          <Link href="/browse" className="text-sm text-foreground hover:text-primary transition-colors">Browse Events</Link>
          <a href="#" className="text-sm text-foreground hover:text-primary transition-colors">Features</a>
          <a href="#" className="text-sm text-foreground hover:text-primary transition-colors">Pricing</a>
        </nav>
            ...
            <div className="relative" ref={menuRef}>
              ...
              {menuOpen && (
                <div className="absolute right-0 top-full mt-2 z-40 w-56 rounded-xl border border-border bg-background shadow-lg py-2">
                  <div className="px-4 py-2 border-b border-border">
                    <p className="text-xs uppercase tracking-wider text-foreground/60">Role</p>
                    <p className="text-sm font-semibold text-foreground">{session.role}</p>
                  </div>
                  {session.role === "ADMIN" && (
                    <Link
                      href="/admin"
