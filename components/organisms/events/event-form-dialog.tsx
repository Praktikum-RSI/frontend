...
const emptyForm: FormState = { name: "", description: "", quota: "100", start_date: "", end_date: "" };

export function EventFormDialog({ open, mode, event, onClose, onSuccess }: EventFormDialogProps) {
  ...
  const validate = (): boolean => {
    ...
    if (!form.description.trim()) next.description = "Deskripsi event wajib diisi";
    const quotaNum = Number(form.quota);
    if (!form.quota || Number.isNaN(quotaNum) || quotaNum <= 0) next.quota = "Kuota harus lebih besar dari 0";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message =
        err instanceof ApiError ? err.message : err instanceof Error ? err.message : "Gagal menyimpan event";
    ...
    >
      <div className="bg-background rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-border shadow-2xl">
        <div className="sticky top-0 bg-background border-b border-border px-8 py-5 flex items-center justify-between z-10">
          <div>
            ...
            <p className="text-sm text-foreground/70 mt-1">
              {mode === "create" ? "Isi detail untuk membuat event baru." : "Perbarui detail event ini."}
            </p>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-5">
          ...
          <div>
            <label className="block text-sm font-semibold text-foreground mb-2">Nama Event</label>
            <input
              type="text"
              value={form.name}
          ...
          <div>
            <label className="block text-sm font-semibold text-foreground mb-2">Deskripsi</label>
            <textarea
          ...
          <div>
            <label className="block text-sm font-semibold text-foreground mb-2">Kuota Peserta</label>
            <input
              type="number"
              min={1}
              value={form.quota}
              ...
              className="w-full px-4 py-3 rounded-lg border-2 border-border bg-muted/30 text-foreground placeholder-muted-foreground focus:outline-none focus:border-primary transition-colors"
            />
            {errors.quota && <p className="text-destructive text-sm mt-1">{errors.quota}</p>}
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              />
              {errors.start_date && <p className="text-destructive text-sm mt-1">{errors.start_date}</p>}
            </div>
            <div>
              <label className="block text-sm font-semibold text-foreground mb-2">Tanggal Selesai</label>
              <input
                type="datetime-local"
                value={form.end_date}
                ...
                className="w-full px-4 py-3 rounded-lg border-2 border-border bg-muted/30 text-foreground focus:outline-none focus:border-primary transition-colors"
              />
              {errors.end_date && <p className="text-destructive text-sm mt-1">{errors.end_date}</p>}
            </div>
          </div>
          <div className="flex flex-col-reverse sm:flex-row gap-3 pt-6 border-t border-border">
            ...
            <Button type="submit" disabled={submitting} className="flex-1 bg-primary text-primary-foreground hover:bg-primary/90">
              {submitting ? "Menyimpan..." : mode === "create" ? "Buat Event" : "Simpan Perubahan"}
            </Button>
          </div>
        </form>
      </div>
