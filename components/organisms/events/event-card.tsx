...
import { type Event, deriveEventStatus, formatEventDateRange } from "@/lib/events";
...
export function EventCard({ event, variant = "public", onEdit, onDelete }: EventCardProps) {
    ...
    return (
      <div className="flex items-center gap-4 rounded-lg border border-border bg-card p-4 hover:bg-muted transition-colors">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-foreground/70 line-clamp-1">{dateLabel}</p>
        </div>
        ...
        <div className="flex items-center gap-2">
          {onEdit && (
            <Button size="sm" variant="ghost" onClick={() => onEdit(event)} aria-label="Edit event">
  ...
  return (
    ...
    >
      ...
      <div className="p-6 flex flex-col flex-1">
        ...
        <p className="text-sm text-foreground/70 line-clamp-2 mb-4">{event.description}</p>
